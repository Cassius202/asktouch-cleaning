...
import { FaLinkedin, FaInstagram, FaTwitter, FaTiktok, FaWhatsapp, FaPhone } from "react-icons/fa";

const Footer = () => {
  return (
    <div className="pt-20 px-4">
      <footer className="bg-zinc-900 w-full max-w-[1350px] mx-auto text-white pt-8 lg:pt-12 px-4 sm:px-8 md:px-16 lg:px-28 rounded-tl-3xl rounded-tr-3xl overflow-hidden">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-6 gap-8 md:gap-12">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <p className="text-sm/6 text-gray-300 max-w-96">
              {askTouchData.brand.slogan}. Creating healthy, pest-free spaces for homes and offices across {askTouchData.contact.locations.join(' and ')}.
            </p>
          ...
          <div className="lg:col-span-3 grid grid-cols-2 md:grid-cols-3 gap-8 md:gap-12 lg:gap-16 items-start">
            ...
            {/* Services */}
            <div>
              <h3 className="font-semibold text-sm mb-4 text-white">Services</h3>
              <ul className="space-y-3 text-sm text-gray-300">
                {askTouchData.services.map((service, index) => (
                  <li key={index}>
                    <Link 
                      href={`/services/${service.toLowerCase().replace(/\s+&\s+/g, '-and-').replace(/\s+/g, '-')}`}
            ...
            <div className="col-span-2 md:col-span-1">
              <h3 className="font-semibold text-sm mb-4 text-white">Contact Us</h3>
              <ul className="space-y-3 text-sm text-gray-300">
                {askTouchData.contact.phone_numbers.map((phone, index) => (
                  <li key={index} className="flex items-center gap-2">
                    <FaPhone size={12} className="text-emerald-400" />
                    <a href={`tel:+234${phone.slice(1)}`} className="hover:text-emerald-400 transition-colors">
                      {phone}
                    </a>
                  </li>
                ))}
